...
def get_predictions(node_df,prediction_horizon,num_trajectories):
    ...
    env.attention_radius = attention_radius
    env.robot_type = env.NodeType.VEHICLE
    node_df['type']= env.NodeType.PEDESTRIAN

    done_scene=generate_env_with_scene_from_dataframe(node_df,0.5)
    scenes = []
    ...
    #Initialise a main dictionary for all predictions 
    full_prediction_dict = dict()
    for timestep in range(init_timestep + 1, eval_scene.timesteps):
        ...
        plt.close(fig)
        full_prediction_dict["@t=" + str(timestep)] = prediction_dict_at_timestep
        break
    return full_prediction_dict

def convert_traj_to_mean_stdev(input_dict):
    ...
    for timestep in input_dict:
        ...
        for pred_time in input_dict[timestep]:
            predictions = input_dict[timestep][pred_time]
            x_vals = list(np.float_(predictions[0]))
            y_vals = list(np.float_(predictions[1]))
            dict_with_mean_stdev[timestep][pred_time] = [[statistics.mean(x_vals),statistics.stdev(x_vals)],[statistics.mean(y_vals),statistics.stdev(y_vals)]]
...
if __name__ == '__main__':
    ## setting up the custom environment parameters
    ...
    node_df = pd.DataFrame({'frame_id': frame_id,
                                        ...
                                        'height': 0.3,
                                        'heading': 0})
    get_predictions(node_df,3,2)
    ## get_predictions takes in the trajectory of the object, followed by the prediction horizon 
    ## (aka how far into the future you wish to predict)
    ## followed by the number of trajectories you wish to predict
    ## returns {particular timestep: dictionary of predictions made at the timestep}, where
    ## dictionary of predictions are {timestep: [predicted x values for diff trajectories],[predicted y values for diff trajectories]} 
