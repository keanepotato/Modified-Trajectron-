...
### Calculate scene graph according to the positions of the different entities 
### in the dataset, where nodes are included; leads to a different function to 
### create a scene graph from a dictionary 
    def calculate_scene_graph(self,
        ...
        for node in self.nodes:
            if type(node) is MultiNode:
                ...
                node_pos_dict[node] = np.squeeze(node.get(timestep_range, {'position': ['x', 'y']}))
        #print(timestep_range)


        #### TODO: Instead of create from temp scene dict, use the new function which utilises new criteria 
