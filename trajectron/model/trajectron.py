...
class Trajectron(object):
    ...
    def predict(self,
        ...
        for node_type in self.env.NodeType:
            ...
            #call the CVAE from the model
            model = self.node_models_dict[node_type]

            # Get Input data for node type and given timesteps
            # aka obtain all the position, velocity, acceleration data from the particular scene
            ...
            for i, ts in enumerate(timesteps_o): #obtain timesteps from the particular scene
                if ts not in predictions_dict.keys():
                    predictions_dict[ts] = dict()

                #for a particular timestep and a node, put in the associated information 
                # structure appears to be in the form of {ts:{node:[array of prediction info]}}   
                predictions_dict[ts][nodes[i]] = np.transpose(predictions_np[:, [i]], (1, 0, 2, 3))

        #appears that we have an array of our possible predictions
