...
def process_scene(ns_scene, env, nusc, data_path):
    ...
    #standardising the data, generating min and max such that you can make the x & y-reference lower
    #as well as describe the size of the map
    ...
    scene.map = type_map #put the map into the scene
    del map_mask
    ...
    for node_id in pd.unique(data['node_id']):
        ...
        print(type(node_df.iloc[0]['type']))
...
def process_data(data_path, version, output_path, val_split):
    ...
    for data_class in ['train', 'val', 'test']:
        ...
        for ns_scene_name in tqdm(ns_scene_names[data_class]):
            ...
            #process the scene accordingly, where ns_scene refers to the exact scene that 
            # you are processing, env refers to the parameters you are processing with
            # nusc refers to the collection of scenes 
            scene = process_scene(ns_scene, env, nusc, data_path)
         
            if scene is not None: #augment the scene if the scene is not none and you are in training mode
                if data_class == 'train':
